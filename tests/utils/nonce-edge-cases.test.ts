...
import { validateNonce, generateNonce } from "../../src/utils/nonce.js";
...
describe("nonce - Edge Cases", () => {
  ...
  it("should handle nonce with zero timestamp", () => {
    ...
    expect(() => validateNonce(zeroNonce)).toThrow("Invalid timestamp in nonce");
  });
  ...
  it("should handle nonce with non-numeric timestamp", () => {
    ...
    expect(() => validateNonce(invalidNonce)).toThrow("Invalid timestamp in nonce");
  });
});
